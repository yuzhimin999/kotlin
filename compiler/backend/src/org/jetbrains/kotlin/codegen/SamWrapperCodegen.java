...
public class SamWrapperCodegen {
    ...
    public Type genWrapper(
        ...
        if (isKotlinFunInterface) {
            ...
            generateEquals(cv, asmType, samAsmType);
            generateHashCode(cv, asmType);
        }
    ...
    private static void generateEquals(@NotNull ClassBuilder cv, @NotNull Type asmType, @NotNull Type samAsmType) {
        ...
        iv.load(0, OBJECT_TYPE);
        iv.invokevirtual(asmType.getInternalName(), GET_FUNCTION_DELEGATE.getName(), GET_FUNCTION_DELEGATE.getDescriptor(), false);
        iv.load(1, OBJECT_TYPE);
    ...
    private static void generateHashCode(@NotNull ClassBuilder cv, @NotNull Type asmType) {
        MethodVisitor mv = cv.newMethod(JvmDeclarationOrigin.NO_ORIGIN, ACC_PUBLIC, "hashCode", "()I", null, null);
        InstructionAdapter iv = new InstructionAdapter(mv);
        iv.load(0, OBJECT_TYPE);
        iv.invokevirtual(asmType.getInternalName(), "getFunctionDelegate", "()Lkotlin/Function;", false);
