...
internal class FunctionReferenceLowering(private val context: JvmBackendContext) : FileLoweringPass, IrElementTransformerVoidWithContext() {
    ...
    private inner class FunctionReferenceBuilder(val irFunctionReference: IrFunctionReference, val samSuperType: IrType? = null) {
        ...
        fun build(): IrExpression = context.createJvmIrBuilder(currentScope!!.scope.scopeOwnerSymbol).run {
            irBlock {
                ...
                if (!isLambda && samSuperType == null && !useOptimizedSuperClass) {
                    createLegacyMethodOverride(irSymbols.functionReferenceGetSignature.owner) { generateSignature() }
                    createLegacyMethodOverride(irSymbols.functionReferenceGetName.owner) { generateName() }
                    createLegacyMethodOverride(irSymbols.functionReferenceGetOwner.owner) { calculateOwner(callee.parent, backendContext) }
                }
            ...
            functionReferenceClass.addConstructor {
                ...
                val constructor = if (samSuperType != null) {
                    ...
                    val expectedArity = when {
                        isLambda -> 1
                        useOptimizedSuperClass ->
                            if (boundReceiver != null) 6 else 5
                        else ->
                            if (boundReceiver != null) 2 else 1
                    }
                        ...
                        +irDelegatingConstructorCall(constructor).apply {
                            if (samSuperType == null) {
                                ...
                                if (!isLambda && useOptimizedSuperClass) {
                                    ...
                                    putValueArgument(index++, generateName())
                                    putValueArgument(index++, generateSignature())
                                    // TODO: record real parents of adapted references in a separate pass before this lowering,
                                    //       and use them here and in generateOwner()
        ...
        private fun JvmIrBuilder.generateName(): IrExpression =
